...
import { Attributes, fdmgObject } from './elements/utils';
...
export const parseXML = (xmlString: string) => {
  ...
  const json: Array<ReturnType<typeof mapElement>> = [].slice
    ...
    .map((childNode: Element, i: number) => {
  ...
  if (element.childNodes) {
    if (element.childNodes.length === 1 && element.childNodes[0].nodeName === "#text") {
      ...
      for (let i = 0; i < element.childNodes.length; i++) {
        const elementContent = new XMLSerializer().serializeToString(element.childNodes[i]);
