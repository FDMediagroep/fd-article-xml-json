...
export function parseXMLToJSON(doc: string) {
  ...
  const articleContentJSON: Object[] = [];
  [].slice
      ...
      switch (childNode.nodeName) {
        case "fdmg-bulletpoint":
          ...
          articleContentJSON.push({
            ...
            key: idx,
            bullets: decodeHtml(innerHTML(childNode, "fdmg-content")).split(
              "\n"
            ),
        ...
        case "fdmg-text-frame":
          if (childNode?.getElementsByTagName("fdmg-filename")?.length) {
            fileName = innerHTML(childNode, "fdmg-filename");
          }
          articleContentJSON.push({
            ...
            image: fileName,
            title: innerHTML(childNode, "fdmg-heading"),
            descriptions: innerHTML(childNode, "fdmg-content").split("\n"),
            alignment: innerHTML(childNode, "fdmg-alignment"),
          });
          break;
        case "fdmg-twitter":
        ...
        default:
          ...
          if (childNode?.nodeName?.indexOf("fdmg-") === -1) {
            ...
            console.log(new XMLSerializer().serializeToString(childNode));
          }
      }
    });
